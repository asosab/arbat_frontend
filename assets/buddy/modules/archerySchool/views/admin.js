...
(function (window, document) {
  ...
  window.BuddyArcherySchoolViews.admin=function(context){
    ...
    function schoolEquipmentMapper(item){return {value:String(equipmentId(item)),label:equipmentLabel(item),keywords:[item&&item.tipo,item&&item.marca,item&&item.modelo,item&&item.numeroSerie,item&&item.estado].filter(Boolean).map(function(v){return String(v).toLowerCase();})};}
