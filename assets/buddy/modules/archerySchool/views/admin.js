...
(function (window, document) {
  ...
  window.BuddyArcherySchoolViews.admin=function(context){
    ...
    var pl=document.createElement('label');pl.textContent='Estudiante registrado';var ps=document.createElement('select');ps.name='personaId';ps.required=true;pl.appendChild(ps);pf.appendChild(pl);
    var pn=add(pf,'Nombre completo','nombreCompleto',state.profile&&state.profile.nombreCompleto,'text',true);add(pf,'Fecha de nacimiento','fechaNacimiento',state.profile&&state.profile.fechaNacimiento,'date',false);
    var pa=add(pf,'Perfil activo','activo','', 'checkbox',false);pa.checked=state.profile?state.profile.activo!==false:true;
    var psa=actions(pf,'Guardar perfil');profile.appendChild(pf);
    pf.addEventListener('submit',function(e){e.preventDefault();psa.button.disabled=true;api.updateProfile({nombreCompleto:pn.value.trim(),fechaNacimiento:pf.elements.fechaNacimiento.value||null,activo:pa.checked}).then(function(){psa.status.textContent='Perfil guardado.';}).catch(function(err){psa.status.textContent=err.message;}).finally(function(){psa.button.disabled=false;});});
    root.appendChild(profile);

    /* INSCRIPCIÓN */
    var enrollment=document.createElement('section'),eh=document.createElement('h3');eh.textContent='Inscripción';enrollment.appendChild(eh);var ef=document.createElement('form');
    add(ef,'Sitio','sitio',state.enrollment&&state.enrollment.sitio||config.siteId,'text',true);sel(ef,'Estado','estado',config.enrollmentStates||[],state.enrollment&&state.enrollment.estado||'activo',true);add(ef,'Fecha de inscripción','fechaInscripcion',state.enrollment&&state.enrollment.fechaInscripcion,'date',true);
    var ea=actions(ef,state.enrollment?'Guardar inscripción':'Registrar inscripción');enrollment.appendChild(ef);
    ef.addEventListener('submit',function(e){e.preventDefault();var personaId=ps.value;if(!personaId){ea.status.textContent='Selecciona un estudiante.';return;}ea.button.disabled=true;var data={personaId:personaId,sitio:ef.elements.sitio.value.trim(),estado:ef.elements.estado.value,fechaInscripcion:ef.elements.fechaInscripcion.value||null};(state.enrollment?api.updateEnrollment(data):api.createEnrollment(data)).then(function(){ea.status.textContent='Inscripción guardada.';}).catch(function(err){ea.status.textContent=err.message;}).finally(function(){ea.button.disabled=false;});});
    ...
    function fillStudents(){
      setOptions(ps,students,'Selecciona un estudiante',function(s){return {value:String(studentId(s)),label:studentName(s)};});
      setOptions(recS,students,'Selecciona un estudiante',function(s){return {value:String(studentId(s)),label:studentName(s)};});
      setOptions(ownerS,buildOwnerOptions(),'Selecciona el propietario',function(o){return {value:ownerKey(o),label:ownerLabel(o)};});
    }
    ps.addEventListener('change',function(){var s=studentById(ps.value);if(s){pn.value=s.nombreCompleto||s.name||'';pf.elements.fechaNacimiento.value=s.fechaNacimiento||'';pa.checked=s.activo!==false;}});

    api.getStudents().then(function(list){students=Array.isArray(list)?list:[];fillStudents();var initial=state.profile&&(state.profile.id||state.profile._id);if(initial&&studentById(initial)){ps.value=initial;ps.dispatchEvent(new Event('change'));}return Promise.all([loadSchool(),refreshStudentList()]);}).catch(function(err){studentGrid.innerHTML='<div class="empty">No se pudo cargar la lista de estudiantes: '+err.message+'</div>';});
