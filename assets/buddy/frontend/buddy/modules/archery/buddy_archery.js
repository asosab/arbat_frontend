...
function getAuthForNameCapture() {
  ...
  function cancelPendingAuthForName() {
    pendingAuthForName = false;
  }

  function isArcheryHandlingAuthWelcome() {
    return !!pendingAuthForName;
  }

  function handleAuthenticationForName(event) {
    if (!pendingAuthForName) return;
    var detail = event && event.detail ? event.detail : {};
    if (!detail.authenticated) return;

    // El usuario ya se autenticó. Ahora, y solo ahora, pasamos al segundo paso:
    // solicitar el nombre si la cuenta todavía no lo tiene.
    pendingAuthForName = false;
    if (detail.needsName || getAuthForNameCapture()) {
      startNameCapture();
    }
  }

  function startNameCapture() {
    var auth = getAuthForNameCapture();
    var chat = window.Buddy && window.Buddy.chat;
    if (!auth || !chat || typeof chat.open !== 'function' || typeof chat.setInteraction !== 'function') return false;

    nameCaptureActive = true;
    chat.open();
    if (typeof chat.setPlaceholder === 'function') chat.setPlaceholder('Escribe tu nombre');
    if (typeof chat.clearInput === 'function') chat.clearInput();

    chat.setInteraction(function (texto) {
      var value = String(texto || '').trim();
      if (!value) return false;

      // Los comandos conocidos no se interpretan como nombres: cancelan el
      // flujo y vuelven a pasar por el intérprete normal del chat.
      var command = value.toLowerCase().replace(/[áéíóúüñ]/g, function (c) {
        return ({'á':'a','é':'e','í':'i','ó':'o','ú':'u','ü':'u','ñ':'n'})[c] || c;
      });
      if (command === 'hola' || command === 'reservas') {
        cancelNameCapture();
        return typeof chat.send === 'function' ? chat.send(value) : false;
      }

      nameCaptureActive = false;
      var firstWord = value.split(/\s+/)[0];
      return auth.registerName(value).then(function () {
        if (typeof chat.restorePlaceholder === 'function') chat.restorePlaceholder();
        if (typeof chat.clearInteraction === 'function') chat.clearInteraction();
        say('¡Hola ' + firstWord + '! desde ahora podré llamarte por tu nombre y recordar tus flechas jugadas', 'alegre');
        return true;
      }).catch(function () {
        // Si falla el registro, mantenemos el flujo activo para que pueda
    ...
    if (typeof chat.focusInput === 'function') chat.focusInput();
    say('Dame tu nombre para anotarte tus puntos', 'sereno');
    return true;
  }
  ...
  function narrateAndanadaTotal(total) {
    ...
    andanadaBubbleTimer = setTimeout(function () {
      ...
      // Después de anunciar la suma:
      // - usuario autenticado sin nombre: se solicita el nombre.
      // - usuario no autenticado: primero se invita a autenticarse; el nombre
      //   solo se solicita después de que Auth confirme la autenticación.
      var auth = window.Buddy && window.Buddy.auth;
      var isAuthenticated = !!(auth &&
        typeof auth.isAuthenticated === 'function' &&
        auth.isAuthenticated());

      if (getAuthForNameCapture()) {
        startNameCapture();
...
function init() {
  ...
  window.addEventListener('buddy:chat-closed', cancelPendingAuthForName);
