...
(function (window, document) {
  ...
  function closeChat() {
    ...
    clearInteraction();
    try {
      window.dispatchEvent(new CustomEvent('buddy:chat-closed'));
    } catch (e) {}
    return true;
  }
  ...
  function showWelcome(authState) {
    // Archery puede tener un flujo post-andanada pendiente: en ese caso la
    // autenticación es el primer paso y Archery solicitará el nombre después
    // de recibir buddy:auth-state-changed. Evitamos mostrar simultáneamente
    // el saludo genérico de nuevo usuario de Chat.
    if (window.Buddy && window.Buddy.archery &&
        typeof window.Buddy.archery.isHandlingAuthWelcome === 'function' &&
        window.Buddy.archery.isHandlingAuthWelcome()) return;
